...
impl MeApp {
    ...
    pub(super) fn organize_files(&mut self, cx: &mut Context<Self>) {
        if !self.ai_ready() || self.organize_cancel.is_some() || self.organization_attempted {
            return;
        }
        ...
        let task = cx.background_executor().spawn(async move {
            loop {
                ...
                let folders = me_agent::codex::organize_documents(input, cancel.clone())?;
                if folders.is_empty() {
                ...
                if folders.len() != allowed.len() {
                    return Err("Not all files could be organized.".into());
                }
                session
                    .lock()
                    .map_err(|_| "Vault unavailable.")?
