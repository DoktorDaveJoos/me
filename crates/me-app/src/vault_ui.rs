...
impl MeApp {
    ...
    pub(super) fn lock_vault(
        ...
        let old = std::mem::replace(&mut self.session, Arc::new(Mutex::new(None)));
        let generation = self.generation;
        let task = cx.background_executor().spawn(async move {
    ...
    pub(super) fn locked_view(&self, window: &Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        div()
            ...
            .child(
                div()
                    ...
                    .when(!self.busy && !self.initialized && !restoring, |s| {
                        s.child(self.editor_field(
                            "Repeat password",
    ...
    pub(super) fn document_modal(&self, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        self.overlay(cx).child(
            modal_panel(550.)
                ...
                .when(supports_text, |s| {
                    s.child(div().type_style(Type::Small).text_color(rgb(MUTED)).child(
                        "AI analysis sends extracted text to OpenAI using your ChatGPT account.",
                    ))
                    .child(
                        div()
                            .id("extract-document")
