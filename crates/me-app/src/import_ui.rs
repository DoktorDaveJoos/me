...
impl MeApp {
    ...
    pub(super) fn import_confirmation(&self, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        self.overlay(cx).child(modal_panel(620.).max_h_full()
            ...
            .child(div().type_style(Type::Small).text_color(rgb(MUTED)).child(if self.pending_attach_to_search {"The selected files will be saved and attached in Search. Form text is analyzed with your connected ChatGPT account."} else if self.settings.automatic_evaluation {"Originals stay in your vault. Extracted content is analyzed with your connected ChatGPT account. Results remain suggestions for review."} else {"Originals stay in your vault. Automatic analysis is off; you can start it for each file in Imports."}))
    ...
    pub(super) fn import_view(&self, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        div().id("import-page").flex_1().min_h_0().overflow_y_scroll().px(px(space::PAGE)).pb(px(space::PAGE))
            .child(div().max_w(px(layout::SEARCH_WIDTH)).mx_auto().pt(px(space::PAGE_TOP)).flex().flex_col().gap(px(space::XXL))
                ...
                .child(div().type_style(Type::Body).text_color(rgb(MUTED)).child("Drop files anywhere in ME. Follow each original from reading to review."))
            ...
            .children(self.intake_failures.iter().enumerate().map(|(index,(path,error))| {
                ...
                .child(div().type_style(Type::Caption).text_color(rgb(MUTED)).child("Context uses local document guidance. No private content is searched on the web. Uncertain details stay visible for review.")))
    }
    fn import_row(&self, job: &ImportJob, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let status = if let Some(a) = active {
            format!("{} · {}s", stage.label(), a.started.elapsed().as_secs())
        } else if !job.processable {
            "Original saved · conversion needed".into()
        } else {
            match job.state.as_str() {
                "failed" => format!("Stopped at {}", stage.label()),
                "done" if job.warning.is_some() => "Needs attention".into(),
                "done" if job.proposals == 0 && job.questions == 0 => {
                    "Analysis complete · no pending suggestions".into()
                }
                "done" => format!(
                    "{} details ready for review · {} questions",
                    job.proposals, job.questions
                ),
                "manual" => "Waiting for your approval".into(),
                _ => "Queued · original saved".into(),
            }
        };
        let color = if active.is_some() {
            ...
            DANGER
        } else if job.warning.is_some() || !job.processable {
            WARNING
        ...
        let (current, total) = active
            .map(|a| (a.current, a.total))
            .unwrap_or((job.current, job.total));
        div()
            .p(px(space::LG))
            .bg(rgb(SURFACE))
                ...
                div()
                    ...
                    .child(
                        div()
                            ...
                            .flex_col()
                            .gap(px(space::XS))
                            .child(div().type_style(Type::Label).child(job.title.clone()))
                            .child(
                                div()
                    ...
                    .child(
                        div()
                            .id(SharedString::from(format!("import-open-{item}")))
                            .type_style(Type::Small)
            ...
            .when(active.is_some(), |s| {
                s.child(div().flex().flex_wrap().gap(px(space::MD)).children(
                    ImportStage::STEPS.iter().map(|step| {
                        div()
                            .type_style(Type::Caption)
                            .text_color(rgb(if *step == stage { ACCENT } else { MUTED }))
                            .child(step.label())
                    }),
                ))
            })
            .when_some(active.map(|a| a.message.clone()), |s, m| {
                s.child(
                    div()
                        .type_style(Type::Small)
            ...
            .when(active.is_some() && total > 0, |s| {
                s.child(
                    div()
                        ...
                        .child(format!(
                            "{} {current} of {total}",
                            if stage == ImportStage::Normalizing {
                                "Page"
                            } else {
                                "Section"
                            }
            ...
            .when_some(job.error.clone().or_else(|| job.warning.clone()), |s, m| {
                s.child(
                    div()
                        .type_style(Type::Small)
                        .text_color(rgb(color))
            ...
            .when(active.is_some(), |s| {
                s.child(
                    div()
                        .id(SharedString::from(format!("stop-import-{item}")))
                        .type_style(Type::Small)
                        .cursor_pointer()
                        .on_click(cx.listener(move |this, _, _, cx| {
                            if let Some(active) = this.active_imports.get_mut(&item) {
                                active.cancel.store(true, Ordering::SeqCst);
                                active.message = "Stopping; keeping completed sections…".into();
                            }
                            cx.notify();
                        }))
                        .child("Stop"),
            ...
            .when(
                active.is_none()
                    && job.processable
                    && matches!(job.state.as_str(), "failed" | "manual"),
                |s| {
                    s.child(
                        div()
                            .id(SharedString::from(format!("retry-import-{item}")))
                            .type_style(Type::Small)
                            .text_color(rgb(ACCENT))
                            .cursor_pointer()
                            .on_click(cx.listener(move |this, _, _, cx| this.run_ai(item, cx)))
                            .child(if self.active_imports.len() >= MAX_ACTIVE_IMPORTS {
                                "Workers busy · try shortly"
