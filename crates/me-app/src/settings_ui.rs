...
impl MeApp {
    ...
    pub(super) fn settings_view(&self, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        div().id("settings-page").flex_1().min_h_0().pt(px(space::PAGE_TOP)).bg(rgb(BG)).font_family(font::SANS).type_style(Type::Body).text_color(rgb(INK))
            ...
            .child(div().id("settings-scroll").flex_1().min_h_0().overflow_y_scroll().px(px(space::PAGE)).pb(px(space::PAGE))
                .child(div().max_w(px(layout::CONTENT_WIDTH)).mx_auto().flex().flex_col().gap(px(space::XXXL))
                    ...
                    .child(div().py(px(space::LG)).border_b_1().border_color(rgb(LINE)).flex().items_start().gap(px(space::XXL))
                        .child(div().flex_1().flex().flex_col().gap(px(space::SM))
                            .child(div().type_style(Type::Label).child("Read new files automatically"))
                            .child(div().type_style(Type::Small).text_color(rgb(MUTED)).child("Extracted text is sent to OpenAI using your ChatGPT account. Turn off to approve each file first.")))
                        .child(div().id("automatic-evaluation-toggle").w(px(54.)).h(px(28.)).rounded_full().bg(rgb(if automatic{ACCENT}else{LINE})).text_color(rgb(if automatic{SURFACE}else{INK})).type_style(Type::Caption).flex().items_center().justify_center().cursor_pointer()
                    ...
                    .child(div().flex().items_center().justify_between()
                        .child(div().type_style(Type::Body).child("ChatGPT connected"))
                        .child(div().id("settings-check-codex").type_style(Type::Small).text_color(rgb(ACCENT)).cursor_pointer()
                            .on_click(cx.listener(|this,_,_,cx|{if this.active_imports.is_empty() && this.filter.cancel.is_none(){this.start_codex_setup(false,cx);}})).child("Check connection")))
                    ...
                    .when_some(self.notice.clone(),|s,m|s.child(div().type_style(Type::Small).text_color(rgb(MUTED)).child(m)))))
    }
}
