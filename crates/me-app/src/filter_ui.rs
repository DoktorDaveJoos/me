...
impl MeApp {
    ...
    pub(super) fn filter_changed(&mut self, cx: &mut Context<Self>) {
        ...
        self.filter.revision += 1;
        self.filter.loading = !self.filter.query.is_empty();
        let revision = self.filter.revision;
        if self.app_ready() && !self.filter.query.is_empty() {
                        ...
                        .when(rows.len() > 60, |s| {
                            s.child(
                                div()
                                    ...
                                    .type_style(Type::Caption)
                                    .text_color(rgb(MUTED))
                                    .child("Showing 60 matches. Refine your search."),
                            )
                        })
                    })
                    .when(form, |s| {
