-- Initial encrypted local schema, derived from the 2026-09-14 architecture.
...
CREATE TABLE job (
  ...
  state TEXT NOT NULL CHECK(state IN ('queued','running','waiting_provider','needs_review','waiting_extractor','done','failed')),
...
CREATE TABLE vault_meta (
  ...
  profile_id TEXT NOT NULL REFERENCES entity(id)
) STRICT;
...
-- Local journal only: NOT a shipped sync wire protocol or causal merge strategy.
CREATE TABLE local_change (
